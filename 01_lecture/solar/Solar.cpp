// Kompajliranje:
// g++ -o Solar Solar.cpp util_t.cpp -lGLEW -lGL -lGLU -lglut -lpthread
...
void myDisplay()
{
	...
	if (spinMode) {
 	...
 	// Our ModelViewProjection (for the sun) : multiplication of our 2 matrices
...
// Zemlja!!!
	...
	// The ModelViewProjection matrix for the earth
...
// Mjesec!!!
	...
	// The ModelViewProjection matrix for the moon
	mvp = projection * model;
	det=60.0f;
	glUniform1f(DetailID,det); 
	glUniformMatrix4fv(MVPMatrixID, 1, GL_FALSE, &mvp[0][0]);
