...
vec4 LightAtPoint (vec4 Position, vec3 Normal)
{
	...
	vec3 twoNormal = Normal;
	if (TwoSided && dot(Normal,EyeVector)<0.0) 
...
void main() {
	...
	vec3 v1 = v[1].xyz/v[1].w - v[0].xyz/v[0].w; 
    vec3 v2 = v[2].xyz/v[2].w - v[0].xyz/v[0].w; 
	
	vec4 Center = (v[0]+v[1]+v[2])/3.0;
	vec3 Normal = normalize(cross(v1, v2));

	vec4 CenterColor = LightAtPoint (Center, Normal);
	
	for (i=0; i<gl_in.length(); i++)
	{
		gl_Position = PMatrix*v[i];
		PatchColor = CenterColor;
		EmitVertex();
	}
	EndPrimitive();
}
